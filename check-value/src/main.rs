// src/main.rs
// CLI entrypoint for check-value comparisons.
...
async fn main() {
    ...
    let code = match run().await {
        Ok(code) => code,
        Err(err) => {
            eprint!("{}", render_error(mode, "check-value", &format!("{err:#}")));
            3
        }
    };
    process::exit(code);
...
async fn handle_compare(
    ...
    Ok(check_value::exit_code(&report))
}
...
fn parse_raw_nodes(raw_node: &[String]) -> Result<Vec<check_value::RawNodeInput>> {
    raw_node
        .iter()
