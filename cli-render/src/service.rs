...
pub fn render_node_server_stopped(mode: OutputMode, node_id: &str, bind_addr: &str) -> String {
    let inner = match mode {
        OutputMode::Pretty => {
            let t = title(mode, "refinery-node serve");
            ...
            format!(
                ...
                key_value(mode, "node_id", node_id),
                key_value(mode, "bind_addr", bind_addr),
            )
        }
        OutputMode::Plain => {
...
pub fn render_database_view_stopped(mode: OutputMode, bind_addr: &str) -> String {
    let inner = match mode {
        OutputMode::Pretty => {
            let t = title(mode, "database-view serve");
            let badge = status_badge(mode, "offline");
        ...
        OutputMode::Plain => format!("status: offline\nbind_addr: {bind_addr}\n"),
    };
    frame_cli_output(mode, inner)
}
...
pub fn overwrite_service_render(previous: &str, next: &str) -> String {
    ...
    format!("\x1b[{line_count}A\x1b[J{next}")
}
