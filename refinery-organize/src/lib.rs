// src/lib.rs
// Shared organization helpers for the dataset management CLI.
...
// Rebuilds `jsonraw/nodes` from the canonical top-level files in `jsonraw`.
// @param: jsonraw_dir - Canonical raw dataset directory containing the source JSON files
...
pub fn partition_jsonraw(jsonraw_dir: &Path, node_count: usize) -> Result<PartitionSummary> {
    if node_count == 0 {
        bail!("node count must be greater than 0");
    }

    if !jsonraw_dir.is_dir() {
        return Err(anyhow!(
            "jsonraw directory does not exist: {}",
            jsonraw_dir.display()
        ));
    }

    let source_files = collect_source_files(jsonraw_dir)?;
    let nodes_dir = jsonraw_dir.join("nodes");
    ...
    Ok(PartitionSummary {
        source_dir: jsonraw_dir.to_path_buf(),
        nodes_dir,
        node_count,
...
// Collects the canonical source JSON files directly under `jsonraw`.
// @param: jsonraw_dir - Canonical raw dataset directory
// @return: Result<Vec<PathBuf>> - Sorted list of top-level JSON files
fn collect_source_files(jsonraw_dir: &Path) -> Result<Vec<PathBuf>> {
    let mut files = Vec::new();

    for entry in fs::read_dir(jsonraw_dir)
        .with_context(|| format!("failed to read {}", jsonraw_dir.display()))?
    {
        let entry = entry?;
        let path = entry.path();
