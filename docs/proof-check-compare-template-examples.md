# `proof-check compare` template examples

This page gives one `cargo run -p proof-check --release -- compare` example for each supported template using prepared baselines in `data/check-baselines`.
