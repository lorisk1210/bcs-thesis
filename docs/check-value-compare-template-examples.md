# `check-value compare` template examples

This page gives one `cargo run -p check-value --release -- compare` example for each supported template using prepared baselines in `data/check-baselines`.
