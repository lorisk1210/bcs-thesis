...
fn seeded_release_is_deterministic() {
    ...
    let first = release_result_with_seed(&query_result, &config, 42).expect("seeded release");
    let second = release_result_with_seed(&query_result, &config, 42).expect("seeded release");
