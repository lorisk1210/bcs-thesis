...
fn init_schema_creates_release_audit_columns() {
    ...
    assert!(has_column(&conn, "privacy_releases", "release_mode"));
    assert!(has_column(&conn, "query_audit", "released_result_json"));
    assert!(has_column(&conn, "query_audit", "release_mode"));
}
