...
fn encrypted_payload_round_trip_works() {
    ...
    let (nonce, ciphertext) = encrypt_share_payload(&sender_private, &recipient_public, &payload)
        .expect("encrypt should succeed");
    let decrypted = decrypt_share_payload(&recipient_private, &sender_public, &nonce, &ciphertext)
