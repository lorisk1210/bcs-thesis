...
fn subgroup_gender_schema_is_stable() {
    let schema =
        schema_for_query(QueryTemplate::SubgroupEffectEstimate, &json!({})).expect("schema");
...
fn aggregate_local_statistics_preserves_time_to_event_max_days() {
    ...
    let aggregated =
        aggregate_local_statistics(QueryTemplate::TimeToEventProxy, &items).expect("aggregate");
